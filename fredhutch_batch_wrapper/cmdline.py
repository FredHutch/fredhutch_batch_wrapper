...
from fredhutch_batch_wrapper.client import ClientError
...
for help on each subcommand.""".format(parser.prog)
    ...
    parser_cancel.add_argument('--job-id', help='job ID', required=True)
    parser_cancel.add_argument('--reason', help='reason for canceling job', required=True)
    parser_terminate = subparsers.add_parser('terminate', help='terminate a job')
    parser_terminate.set_defaults(func=terminate)
    parser_terminate.add_argument('--job-id', help='job ID', required=True)
    parser_terminate.add_argument('--reason', help='reason for terminating job', required=True)
